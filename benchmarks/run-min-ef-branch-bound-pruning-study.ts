...
const OUTPUT_URL = new URL("./results/min-ef-branch-bound-pruning-study.json", import.meta.url);
...
type CandidateExports = SolverExports & {
  configureMinEfBranchBoundPruning: (enabled: number) => number;
...
const CONTRACT = {
  candidate: "B1-canonical-immediate-cost-bound-with-phase2-success-prepass",
  ...
  comparison: "fresh product vs fresh candidate-off vs fresh candidate-on",
  screeningOnly: true,
} as const;
...
type StudyReport = {
  kind: "min-ef-branch-bound-pruning-study";
  version: 1;
  generatedAt: string;
  ...
  records: Array<{
    scenarioId: string;
    product: SolveRecord;
    candidateOff: CandidateRecord;
    candidateOn: CandidateRecord;
    productVsOff: ReturnType<typeof compareRecords>;
    offVsOn: ReturnType<typeof compareRecords>;
    minEfNodeRatio: number | null;
    combinedNodeRatio: number | null;
...
async function main() {
  const provenance = collectResearchProvenance({
    repoRoot: REPO_ROOT,
    studyId: "min-ef-branch-bound-b1-pruning-screen",
    protocolVersion: 1,
    contract: CONTRACT,
    sourceFiles: [
      "benchmarks/run-min-ef-branch-bound-pruning-study.ts",
  ...
  for (const scenario of SCENARIOS) {
    ...
    const candidateOff = await solveCandidate(candidateModule, scenario, false);
    const candidateOn = await solveCandidate(candidateModule, scenario, true);
    const minEfNodeRatio = ratio(candidateOn.minEfNodeCount, candidateOff.minEfNodeCount);
    const combinedNodeRatio = ratio(candidateOn.combinedNodeCount, candidateOff.minEfNodeCount);
    const elapsedRatio = ratio(candidateOn.elapsedMs, candidateOff.elapsedMs);
    ...
    records.push({
      ...
      candidateOff,
      candidateOn,
      productVsOff: compareRecords(product, candidateOff),
      offVsOn: compareRecords(candidateOff, candidateOn),
      minEfNodeRatio,
      combinedNodeRatio,
      elapsedRatio,
    });
    console.log(
      [
        scenario.id,
        `off=${candidateOff.outcome}/${candidateOff.minEfNodeCount}`,
        `on=${candidateOn.outcome}/${candidateOn.minEfNodeCount}`,
        `prepass=${candidateOn.phase2NodeCount}`,
        `prunes=${candidateOn.appliedPrunes}`,
        `semantic=${compareRecords(candidateOff, candidateOn)}`,
      ].join(" "),
    );
  }
...
async function solveCandidate(
  ...
  if (exports.configureMinEfBranchBoundPruning(pruning ? 1 : 0) !== 1) {
    throw new Error("Candidate rejected the branch-bound configuration.");
  }
  const base = solve(exports, scenario);
  const prepassStates = exports.minEfBranchBoundPrepassStates();
  const phase2NodeCount = pruning ? (exports.statesCount?.() ?? null) : null;
  return {
    ...base,
    phase2NodeCount,
    ...
    combinedNodeCount: base.minEfNodeCount + (phase2NodeCount ?? 0),
  };
}

function solve(exports: SolverExports, scenario: Scenario): SolveRecord {
  exports.configureMemo?.(scenario.phase2MemoTier);
  exports.configureMinEfMemo(scenario.minEfMemoTier);
  ...
  return {
    ...
    phase2NodeCount: exports.statesCount?.() ?? null,
    semantic: status === 0 ? snapshot(exports) : null,
  };
}

function snapshot(exports: SolverExports): SemanticSnapshot {
  return {
