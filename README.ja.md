...
公開`/api/stats`レスポンスには、大成功とキット使用に関する集計のみが含まれます。参照元ホスト、クライアント環境、solver診断、失敗観測は公開レスポンスから除外され、非公開の運用用バケット集計としてのみ管理されます。失敗観測には正確な在庫、IPアドレス、ユーザー識別子、エラー原文、スタックトレースを保存しません。イベント件数はユニークユーザー数を意味しません。
