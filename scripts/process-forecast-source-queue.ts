...
import {
  fetchNaverFeedMetadata,
  fetchNaverSoloHistory,
  fetchNaverStructuredItem,
  parseScheduleEvents,
} from "../forecast-collector/src/naver.ts";
import type {
  ScheduleEvent,
  SourceQueueItem,
  SourceQueueResult,
...
async function processItem(queued: SourceQueueItem): Promise<SourceQueueResult> {
  ...
  try {
    ...
    const item = await fetchNaverStructuredItem(boardId, queued.itemId);
...
async function bootstrapItems() {
  return (await fetchNaverSoloHistory()).map(
    (item): SourceQueueItem => ({
