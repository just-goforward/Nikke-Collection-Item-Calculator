import {
  ...
  function updateLevelButtons() {
    levelGrid.querySelectorAll<HTMLButtonElement>(".level-button").forEach((button) => {
  ...
  function requestWorkerTask(
    type,
    input,
    options: { payload?: Record<string, unknown>; onProgress?: (progress: any) => void } = {},
  ) {
    ...
    if (workerPromise) {
      return workerPromise
        ...
        .catch(() => {
          ...
          return rememberCache(solveCache, key, solve(input, updateProgress), SOLVE_CACHE_LIMIT);
        });
    }
    return Promise.resolve(rememberCache(solveCache, key, solve(input, updateProgress), SOLVE_CACHE_LIMIT));
  }

  function validateBestAvailable(input, runs, onProgress, options: { force?: boolean; seed?: number } = {}) {
    ...
    if (workerPromise) {
      return workerPromise
        ...
        .catch(() => {
          ...
          const result = solve({ ...input, monteCarloRuns: runs, monteCarloSeed: seed }, onProgress).monteCarlo;
    ...
    return Promise.resolve(
      rememberCache(
        validationCache,
        key,
        solve({ ...input, monteCarloRuns: runs, monteCarloSeed: seed }, onProgress).monteCarlo,
  ...
  function showSuccessAttemptModal(context) {
    ...
    const input = overlay.querySelector(".attempt-number-input") as HTMLInputElement;
    const form = overlay.querySelector(".attempt-entry-form") as HTMLFormElement;
    ...
    overlay.addEventListener("click", (event) => {
      ...
      const unknownButton = target.closest("[data-success-attempt='unknown']");
      if (unknownButton) {
        applyAttempt(null);
        return;
      }

      const stepButton = target.closest("[data-attempt-step]") as HTMLElement | null;
      if (stepButton) {
  ...
  function bindEvents() {
    ...
    levelGrid.addEventListener("click", (event) => {
      const button = (event.target as Element).closest(".level-button") as HTMLButtonElement | null;
      if (!button) return;
    ...
    resultBox.addEventListener("click", (event) => {
      const target = event.target as Element;
      const convertButton = target.closest("[data-convert]");
      if (convertButton) {
        applyConvert();
        return;
      }
      const button = target.closest("[data-outcome]") as HTMLElement | null;
      if (!button) return;
      applyOutcome(button.dataset.outcome);
    });

    detailBox.addEventListener("click", (event) => {
      const validationButton = (event.target as Element).closest("[data-validation-run]") as HTMLButtonElement | null;
